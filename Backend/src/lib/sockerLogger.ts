...
export class SocketLogger {
  ...
  nodeStarted(
    ...
    this.emit("node_started", {
      ...
      stepIndex,
      totalSteps,
      config: this.sanitizeConfig(config), // ← ADDED
    });
  }

  // ✅ FIXED: Added nodeType parameter for race condition handling
  nodeCompleted(
    nodeId: string,
    label: string,
    output: any,
  ...
  // ✅ FIXED: Added nodeType parameter
  nodeFailed(
    nodeId: string,
    label: string,
    error: string,
