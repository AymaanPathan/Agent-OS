...
export function resolveTemplateString(
  ...
  // Check if it's a single template variable: {{var.name}}
  ...
  if (singleTemplateMatch) {
    ...
    // ✅ FIX: If value is an array with one element, extract it for string fields
    // This prevents arrays from being passed where strings are expected
    if (
      Array.isArray(value) &&
      value.length === 1 &&
      ...
      console.log(
        "✅ [Template] Auto-extracting single element from array:",
        value[0],
      );
      return value[0];
    }
  ...
  // Check if it contains multiple template variables: "Text {{var.a}} and {{var.b}}"
  const hasTemplates = /\{\{([^}]+)\}\}/g.test(input);

  if (hasTemplates) {
    console.log("🔄 [Template] Multiple template variables detected");
    let result = input;
    ...
    for (const match of matches) {
      ...
      console.log(`  🔍 [Template] Resolving: {{${expr}}}`);

      // Same resolution logic as single template
      if (expr.startsWith("last.")) {
        const path = expr.replace("last.", "");
        value = getDeepValue(context.lastOutput, path);
      } else if (expr.startsWith("step.")) {
      ...
      // For string interpolation, always convert to string
      const replacement = value !== undefined ? String(value) : "";
      result = result.replace(match[0], replacement);

      console.log(`    ✓ [Template] Replaced with: "${replacement}"`);
    }

    console.log("✅ [Template] Multi-template string resolved:", result);
    return result;
  }

  // ✅ CRITICAL FIX: Return plain strings as-is
  // This was the missing piece causing "error-test" to become undefined
  console.log(
    "✅ [Template] Plain string (no templates), returning as-is:",
    input,
  );
  return input;
}
...
export function saveToContext(params: {
  ...
  if (nodeType === "tool.healthCheckScanner" && output.unhealthyContainers) {
    ...
    // ✅ CRITICAL FIX: Handle both string and array cases
    let unhealthyContainers: string[] = [];

    if (typeof output.unhealthyContainers === "string") {
      // Single string case
      unhealthyContainers =
        output.unhealthyContainers.trim().length > 0
          ? [output.unhealthyContainers.trim()]
          : [];
    } else if (Array.isArray(output.unhealthyContainers)) {
      // Array case
      unhealthyContainers = output.unhealthyContainers
        .filter(
    ...
    context.vars.unhealthyContainerNames = unhealthyContainers; // Always an array

    // ✅ CRITICAL: Extract first element, handle empty array
    context.vars.firstUnhealthyContainer =
      unhealthyContainers.length > 0 ? unhealthyContainers[0] : ""; // ← Empty string instead of null!

    context.vars.healthReports = output.reports || [];

    console.log(
      "✅ [SaveContext] firstUnhealthyContainer:",
      context.vars.firstUnhealthyContainer,
    );
    console.log(
      "✅ [SaveContext] Type:",
      typeof context.vars.firstUnhealthyContainer,
    );
  }
  ...
  if (nodeType === "tool.dockerListAll") {
    ...
    // ✅ Extract first, use empty string for empty array
    context.vars.firstUnhealthyContainer =
      unhealthyContainers.length > 0 ? unhealthyContainers[0] : "";
  }
...
export async function executeWorkflow(
  ...
  while (currentNode) {
    ...
    logger.nodeStarted(nodeId, nodeType, label, stepIndex, totalSteps, config);
      ...
      switch (nodeType) {
        // ====================================
        // 🔧 FIXED DOCKER TOOL EXECUTION LOGIC
        // ====================================
        // This replaces the section in executeWorkflow.ts around line 850-900

        // In the switch case for Docker tools:
        ...
        case "agent.aiAnalyzer": {
          ...
          try {
            // 🔥 FIX: Handle containerName vs containerNames based on node type
            let normalizedConfig: any;

            // Bulk operations use containerNames (array)
            if (
              nodeType === "tool.dockerBulkLogs" ||
              nodeType === "tool.dockerBulkRestart"
            ) {
              normalizedConfig = {
                ...config,
                containerNames: (() => {
                  if (!config.containerNames) {
                    console.log(
                      "⚠️ [Tool] No containerNames provided for bulk operation",
                    );
                    return [];
                  }
                  ...
                  // Single string - convert to array for bulk operations
                  const cleaned = [config.containerNames]
                    .filter(
                      (name: any) =>
                        name != null &&
                  ...
                  console.log(
                    "✅ [Tool] Normalized single containerName to array:",
                    cleaned,
                  );
                  return cleaned;
                })(),
              ...
              // Single operations use containerName (string)
              normalizedConfig = {
                ...config,
                containerName: (() => {
                  // First check if containerName is already set
                  if (config.containerName) {
                    if (
                      ...
                      console.log(
                        "✅ [Tool] Using provided containerName:",
                    ...
                    if (
                      Array.isArray(config.containerName) &&
                      config.containerName.length > 0
                    ) {
                      const first = config.containerName[0];
                      if (
                        typeof first === "string" &&
                        first.trim().length > 0
                      ) {
                        console.log(
                          "✅ [Tool] Extracted containerName from array:",
                          first,
                        );
                        return first.trim();
                      }
                  ...
                  // If containerNames array exists, extract first element
                  if (config.containerNames) {
                    if (
                      Array.isArray(config.containerNames) &&
                      config.containerNames.length > 0
                    ) {
                      const first = config.containerNames[0];
                      if (
                        typeof first === "string" &&
                        first.trim().length > 0
                      ) {
                        console.log(
                          "✅ [Tool] Extracted first containerName from containerNames array:",
                          first,
                        );
                        return first.trim();
                      }
                    } else if (
                      typeof config.containerNames === "string" &&
                      config.containerNames.trim().length > 0
                    ) {
                      console.log(
                        "✅ [Tool] Using containerNames string as containerName:",
                        config.containerNames,
                      );
                      return config.containerNames.trim();
                    }
                  }

                  console.log("⚠️ [Tool] No valid containerName found");
                  throw new Error(
                    "containerName is required but was not provided or is invalid",
                  );
                })(),
              };

              // Remove containerNames if it exists to avoid confusion
              delete normalizedConfig.containerNames;
            }
